import {Mesh, MeshLambertMaterial, Object3D, PlaneGeometry, SpotLight, SpotLightHelper} from 'three';
...
export class LightView extends View {
    ...
    private init () {
        ...
        for (const light of lights) {
            ...
            const lightPosition = light.getComponent<PositionComponent>(PositionComponent.TYPE);

            lightMesh.position.set(lightPosition.position.x, lightPosition.position.y, lightPosition.position.z);

            this.mesh.add(lightMesh);

            const spotLight = new SpotLight(0xff00000, 1);

            spotLight.position.set(lightPosition.position.x, lightPosition.position.y, lightPosition.position.z);
