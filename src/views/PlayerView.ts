...
export class PlayerView extends View {
    ...
    private init () {
        ...
        const playerDimension = player.getComponent<DimensionComponent>(DimensionComponent.TYPE);
        const playerPosition = player.getComponent<PositionComponent>(PositionComponent.TYPE);

        const geometry = new PlaneGeometry(playerDimension.height * 2, playerDimension.width * 2);
        ...
        this.mesh.position.set(playerPosition.position.x, playerPosition.position.y, playerPosition.position.z);
    }
    ...
    update (interpolationPercentage: number) {
        ...
        const playerPosition = player.getComponent<PositionComponent>(PositionComponent.TYPE);
        const playerMovement = player.getComponent<MovementComponent>(MovementComponent.TYPE);
